...
from lpipsPyTorch import lpips as lpips_original   # 导入原始函数
from lpipsPyTorch.modules.lpips import LPIPS       # 导入模型类，用于缓存
import json
...
# ---------- 添加缓存机制，避免重复构建 LPIPS 模型 ----------
_lpips_cache = {}

def lpips(x, y, net_type='alex', version='0.1'):
    """带缓存的 LPIPS 计算，完全兼容原始接口"""
    key = (net_type, version, x.device)   # 设备也作为缓存键的一部分
    if key not in _lpips_cache:
        model = LPIPS(net_type, version).to(x.device)
        model.eval()
        ...
        try:
            ...
            for method in os.listdir(test_dir):
                ...
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    ...
                    lpipss.append(lpips(renders[idx], gts[idx], net_type='vgg'))   # 调用缓存的 lpips
                    ms_ssims.append(ms_ssim(renders[idx], gts[idx],data_range=1, size_average=True ))
                    lpipsa.append(lpips(renders[idx], gts[idx], net_type='alex'))  # 调用缓存的 lpips
